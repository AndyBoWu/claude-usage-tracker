#!/usr/bin/env python3
"""
Claude Usage Menu Bar App for macOS - Final Fixed Version
Shows real-time Claude usage statistics in the macOS menu bar
This version correctly parses the conversation file format
"""
...
class ClaudeUsageMenuBarApp(rumps.App):
    def __init__(self):
        ...
        # Build menu without using self.menu assignment to avoid duplicates
        self.menu.clear()
        ...
        # Add items to menu
        self.menu.add(self.today_requests)
        self.menu.add(self.today_cost)
        self.menu.add(None)
        self.menu.add(self.daily_avg)
        self.menu.add(self.monthly_total)
        self.menu.add(None)
        self.menu.add(self.refresh_btn)
        self.menu.add(self.auto_refresh_btn)
        self.menu.add(None)
    ...
    def calculate_cost(self, model, input_tokens, output_tokens, cache_creation_tokens, cache_read_tokens):
    ...
    def calculate_stats(self, all_usage):
        ...
        # Today's stats in PST
        pst_offset = timedelta(hours=-8)
        now_utc = datetime.now(timezone.utc)
        now_pst = now_utc + pst_offset
        today_start_pst = now_pst.replace(hour=0, minute=0, second=0, microsecond=0)
        today_start_utc = today_start_pst - pst_offset
        today_end_utc = today_start_utc + timedelta(days=1)
        
        today_usage = [u for u in all_usage if today_start_utc <= u.timestamp < today_end_utc]
